...
struct Settings {
    int quality = 2;    // 0=720p, 1=1080p, 2=1080p HQ, 3=1440p, 4=1440p HQ
...
void draw_settings(App& app) {
    ...
    std::vector<Row> rows = {
        {"영상 화질", kQualityLabels[app.settings.quality]},
        {"버튼 배치", kMappingLabels[app.settings.mapping]},
        {"진동", kVibrationLabels[app.settings.vibration]},
        {"지역 우회", kRegionLabels[app.settings.region]},
        {"게임 언어", kLanguageLabels[app.settings.language]},
        {"음량",
         std::to_string(static_cast<int>(app.settings.volume * 100 + 0.5f)) + "%"},
        {"영상보정", app.settings.smooth ? "안정" : "기본"},
        {"선명도", kSharpnessLabels[app.settings.sharpness]},
        {"배경화면", app.settings.background_slot == 0
             ? "기본"
             : ("배경 " + std::to_string(app.settings.background_slot))},
    };
    if (!app.consoles.empty())
    ...
    rows.push_back({"해제", app.signout_armed
                                    ? "A 다시 눌러 확인"
                                    : app.gamertag});
    ...
    if (!app.last_exit_step.empty())
        app.gfx.text("debug · last exit reached: " + app.last_exit_step,
                     ...
                     gfx::kFaint);
    if (app.settings_cursor == signout_row)
        draw_hints(app, {{"A", app.signout_armed ? "계정 해제"
                                                 : "해제", true},
                         {"B", "뒤로"}});
    else
                ...
                if (direction != 0 && app.settings_cursor != signout_row) {
                    if (app.settings_cursor == 0)
                        app.settings.quality =
                            (app.settings.quality + direction + kQualityCount) % kQualityCount;
                    ...
                    // "실행 위치" only exists with a linked console;
